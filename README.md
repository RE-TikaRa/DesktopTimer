...
- ✅ 修复时钟模式下“暂停/继续”语义不一致的问题，时钟模式中的暂停项改为禁用
- 🔔 修复设置页对 `sounds/` 相对路径铃声的显示与测试逻辑，避免已选文件显示为“未选择”
- 📍 应用设置、语言预览和取消预览时尽量保留窗口位置，不再无条件重新居中
- 🧭 设置页模式切换判断改为基于模式索引/键值，减少对中英文文本匹配的依赖
- 📦 `setup/setup.iss` 改为使用相对路径，去除本机绝对路径硬编码
- 🛠️ 修复 `tools/Run.bat` 与 `tools/pyinstaller.bat` 的工作目录问题，避免从 `tools` 目录运行时找错入口文件

### v2.0.0 (2026-01-22)
- 🧩 设置页全面改为 PyQt6-Fluent-Widgets 组件，并统一 Fluent 视觉风格
- 🎨 新增主题模式（浅/深/跟随系统），菜单样式同步主题
- 📚 预设管理支持搜索/多选/批量删除/排序模式记忆，并保留手动排序体验
- 🧰 托盘与右键菜单结构优化：模式切换、运行状态提示与“窗口”子菜单
- 🔊 提醒增强：音量控制与 Windows 通知开关
- 🗝️ 计时/动作设置键值改为语言无关形式，避免切换语言导致配置混乱
- ⚡ 主题/语言/预设调整后即时预览与同步更新托盘菜单
